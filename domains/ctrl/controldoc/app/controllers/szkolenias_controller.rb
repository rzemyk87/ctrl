class SzkoleniasController < ApplicationController
  ...
  def biurowe
    ...
    szkolenie_last1 = Szkolenie.limit(1).order('szkolenie_id desc').pluck(:szkolenie_id).map(&:to_i)
...
def nowe_parametry
    params.require(:szkolenie).permit(:szkolenie_id,:prowadzacy,:firma,:forma,:data_waznosci,:data_od,:data_do,:rodzaj_id,:data_wystawienia, :osobas_attributes => [:id, :osoba_imie, :osoba_nazwisko, :osoba_data, :osoba_miejsce, :osoba_ocena,:rodzaj_id, :nr_zaswiadczenia])
end
