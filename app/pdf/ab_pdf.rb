class ABPdf < Prawn::Document
  ...
  def szkolony
  	...
  	move_down 5
  	text "#{@osoba.osoba_imie} #{@osoba.osoba_nazwisko}", :align => :center, size: 18, :style => :bold
  	move_down 5
  	text "urodzony(a) dnia: #{@osoba.osoba_data} r. w #{@osoba.osoba_miejsce}", :align => :center, size: 12
  	move_down 5
  end
...
bounding_box([50, 400], :width => 450, :height => 300) do
	...
	move_down 20
	  if @osoba.rodzaj_id == 1
		text "Nr zaświadczenia wg rejestru <b>#{@osoba.nr_zaswiadczenia}/AB/#{@szkolenie.szkolenie_id}#{@data1}</b>", :align => :left, size: 12, :inline_format => true
      elsif @osoba.rodzaj_id == 2	
   		text "Nr zaświadczenia wg rejestru <b>#{@osoba.nr_zaswiadczenia}/R/#{@szkolenie.szkolenie_id}#{@data1}</b>", :align => :left, size: 12, :inline_format => true
      elsif @osoba.rodzaj_id == 3
   		text "Nr zaświadczenia wg rejestru <b>#{@osoba.nr_zaswiadczenia}/IT/#{@szkolenie.szkolenie_id}#{@data1}</b>", :align => :left, size: 12, :inline_format => true
      else
   		text "Nr zaświadczenia wg rejestru <b>#{@osoba.nr_zaswiadczenia}/K/#{@szkolenie.szkolenie_id}#{@data1}</b>", :align => :left, size: 12, :inline_format => true
      end		
   end 
 end
...
def tabela_zaw_ab
	...
	["1.", "   Wybrane regulacje prawne z zakresu prawa pracy dotyczące bezpieczeństwa i higieny pracy:
		   a) praw i obowiązków pracowników i pracodawców w zakresie higieny pracy,
		      z uwzględnieniem:
