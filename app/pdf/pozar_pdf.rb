class PozarPdf < Prawn::Document
 ...
 def plaster
  ...
  def szkolony
    move_down 40
  	text "Pan(i): <b><color rgb='003da5'>#{@osoba.osoba_imie} #{@osoba.osoba_nazwisko}</color></b>", :align => :left, size: 12, :indent_paragraphs => 60, :inline_format => true
    move_down 5
  end
...
w formie Samokształcenia Kierowanego.", size: 12, :indent_paragraphs => 60
	...
	move_down 20
		text "Nr zaświadczenia wg rejestru #{@osoba.nr_zaswiadczenia}/PPOŻ/#{@szkolenie.szkolenie_id}#{@data1}", :indent_paragraphs => 60, size: 12
